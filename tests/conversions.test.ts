...
describe("RGB to RGB Base - normalized rgb values", () => {
  it("rgb 0-255 -> 0-1", () => {
    expect(
      rgbToNormalizedRgb({ r: 255, g: 255, b: 255, a: undefined }),
    ).toStrictEqual({ r: 1, g: 1, b: 1, a: undefined });
  });
  it("rgba 0-255 -> 0-1", () => {
    expect(
      rgbToNormalizedRgb({ r: 255, g: 255, b: 255, a: 0.7 }),
    ).toStrictEqual({ r: 1, g: 1, b: 1, a: 0.7 });
  });
});
...
describe("color conversions", () => {
  ...
  it("XYZ -> RGB", () => {
    expect(
      xyzToRgbNormalized({ x: 20.276, y: 22.449, z: 5.2356 }),
    ).toMatchObject({
...
describe("OKLAB", () => {
  ...
  it("OKLAB -> XYZ 1", () => {
    expect(okLabToXyz({ l: 1, a: 0, b: 0 })).toMatchObject({
      x: expect.closeTo(0.95, 2),
      y: expect.closeTo(1.0, 2),
      z: expect.closeTo(1.089, 2),
    });
  });
  it("OKLAB -> XYZ 2", () => {
    expect(okLabToXyz({ l: 0.45, a: 1.236, b: -0.019 })).toMatchObject({
  ...
  it("OKLAB -> XYZ 3", () => {
    expect(okLabToXyz({ l: 0.922, a: -0.671, b: 0.263 })).toMatchObject({
  ...
  it("OKLAB -> XYZ 4", () => {
    expect(okLabToXyz({ l: 0.153, a: -1.415, b: -0.449 })).toMatchObject({
      x: expect.closeTo(0.0, 2),
      y: expect.closeTo(0.0, 2),
