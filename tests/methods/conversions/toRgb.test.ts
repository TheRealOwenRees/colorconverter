...
describe("to RGB", () => {
  it("return rgb object when input is rgb string", () => {
    expect(new ColorConvertor("rgb(255, 255, 255)").toRgb()).toMatchObject({
      r: 255,
      g: 255,
      b: 255,
      a: undefined,
  ...
  it("return rgba object when input is rgba string", () => {
    expect(
      new ColorConvertor("rgba(255, 255, 255, 0.7)").toRgb(),
    ).toMatchObject({ r: 255, g: 255, b: 255, a: 0.7 });
  });

  it("return rgb object when input is hex string", () => {
    expect(new ColorConvertor("#ffffff").toRgb()).toMatchObject({
      r: 255,
      g: 255,
      b: 255,
      a: undefined,
  ...
  it("return rgb object when input is hsl string", () => {
    expect(new ColorConvertor("hsl(0, 0%, 50%)").toRgb()).toMatchObject({
      r: 128,
      g: 128,
      b: 128,
    });
  ...
  it("return rgb object when input is hsv string", () => {
    expect(new ColorConvertor("hsv(50, 25.5%, 80%)").toRgb()).toMatchObject({
      r: 204,
      g: 195,
      b: 152,
    });
  ...
  it("OKLCH -> RGB", () => {
    const colorConvertor = new ColorConvertor("oklch(70%, 0.1, 297.88");
